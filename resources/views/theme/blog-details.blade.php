            ...
            <div class="row">
                <div class="col-lg-8">
                    <div class="main_blog_details">
                        <img class="{{ asset('assets') }}/img-fluid w-100" src="{{ asset("storage/blogs/$blog->image") }}"
                            alt="">
                        <a href="#">
                            <h4>{{ $blog->name }}</h4>
                    ...
                    <div class="comment-form">
                        ...
                        <form action="{{ route('comments.store') }}" method="POST">
                            ...
                            @if (session('successPublishedComment'))
                                <div class="alert alert-success">
                                    {{ session('successPublishedComment') }}
                                </div>
                            @endif
                            ...
                            <div class="form-group">
                                ...
                                <textarea class="form-control mb-10" rows="5" name="message" placeholder="Messege" onfocus="this.placeholder = ''"
                                    onblur="this.placeholder = 'Messege'">{{ old('message') }}</textarea>
                                @error('message')
                                    <span class="text-danger">{{ $message }}</span>
                                @enderror
                            </div>
                            ...
                            <div class="form-group text-center text-md-right mt-3">
                                <button type="submit" class="button button--active button-contactForm">Add Comment</button>
                            </div>
                        </form>
                    </div>
